...
namespace twoloop
{
    public class OSNetTransform : OSNetTransformBase
    {
        /// <summary>
        ...
        /// player?
        /// </summary>
        [Tooltip("Does this Transform automatically become the OriginShift focus in Start() if this is the local player?")]
        ...
        public override void OnStartServer()
        {
            ...
            if (!clientAuthority)
            {
                ServerInitializeOffset(new TransformTargetData(NetworkOS.hostOffset, transform.position, transform.rotation, true));
            }
        }
        ...
        public override void OnStartClient()
        {
            ...
            {
                CmdInitializeOffset(new TransformTargetData(OriginShift.LocalOffset, transform.position, transform.rotation, true));
        ...
        {
            return Vector3.Magnitude(OriginShift.LocalOffset.ToVector3() - _targetOffset.ToVector3()) > float.Epsilon;
        }
        ...
        private void Update()
        {
            if(!IsClientWithAuthority && !(isServer && !clientAuthority))
            {
                if (_hasOwnerInitializedOffset)
                {
                    ...
                    var localTargetPosition = OriginShift.RemoteToLocal(_targetOffset, _target.remotePosition);
        ...
        private void SendToServer()
        {
            ...
            if (now > _nextSyncTime)
            {
                ...
                var data = new TransformTargetData(OriginShift.LocalOffset, transform.position, transform.rotation,
                    IsOffsetChanged());
    ...
    {
        public OriginShift.Offset offset;
        public Vector3 remotePosition;
        public Quaternion rotation;
        public bool isOffsetChanged;

        public TransformTargetData(OriginShift.Offset offset, Vector3 remotePosition, Quaternion rotation,
