...
namespace twoloop
{
    public class OSNetTransformV2 : OSNetTransformBase
    {
        /// <summary>
        /// Does this Transform automatically become the OriginShift focus in Start() if this is the local
        /// player?
        /// </summary>
        [Tooltip(
            "Does this Transform automatically become the OriginShift focus in Start() if this is the local player?")]
        ...
        [SyncVar] private OriginShift.Offset _spawnPayloadOffset;

        [SyncVar(hook = nameof(OnTargetChanged))]
        private TransformTargetData _target;

        private OriginShift.Offset _targetOffset;
        ...
        //
        // IMPORTANT: of explicit 'OSNTSnapshot' type instead of 'Snapshot'
        //            interface because List<interface> allocates through boxing
        internal SortedList<double, OSNTSnapshot> serverBuffer = new SortedList<double, OSNTSnapshot>();
        internal SortedList<double, OSNTSnapshot> clientBuffer = new SortedList<double, OSNTSnapshot>();
        ...
        // avoid allocations
        Func<OSNTSnapshot, OSNTSnapshot, double, OSNTSnapshot> Interpolate = OSNTSnapshot.Interpolate;
        ...
        public override void OnStartServer()
        {
            ...
            if (!clientAuthority)
            {
                ServerInitializeOffset(new TransformTargetData(NetworkOS.hostOffset, transform.position,
        ...
        public override void OnStartClient()
        {
            ...
            {
                OriginShift.singleton.focus = transform;
            }
            ...
            if (isLocalPlayer || !isServer)
            {
                ...
                transform.position = OriginShift.RemoteToLocal(NetworkOS.hostOffset, transform.position);
            }
            ...
            {
                CmdInitializeOffset(new TransformTargetData(OriginShift.LocalOffset, transform.position,
                    transform.rotation, true));
        ...
        {
            return Vector3.Magnitude(OriginShift.LocalOffset.ToVector3() - _targetOffset.ToVector3()) >
                   float.Epsilon;
        }
        ...
        // => internal for testing
        protected virtual OSNTSnapshot ConstructSnapshot()
        {
            // NetworkTime.localTime for double precision until Unity has it too
            return new OSNTSnapshot(
        ...
        protected virtual void ApplySnapshot(OSNTSnapshot start, OSNTSnapshot goal, OSNTSnapshot interpolated)
        {
            targetComponent.localPosition = interpolatePosition ? interpolated.position : OriginShift.RemoteToLocal(goal.remoteOffset, goal.position);
        ...
        protected virtual void OnClientToServerSync(TransformTargetData targetData)
        {
            ...
            OSNTSnapshot snapshot = new OSNTSnapshot(
                timestamp,
                NetworkTime.localTime,
                OriginShift.RemoteToLocal(_targetOffset, _target.remotePosition), _target.rotation, OriginShift.LocalOffset
            );
        ...
        void UpdateServer()
        {
                ...
                (!clientAuthority || IsClientWithAuthority))
            {
                ...
                OSNTSnapshot snapshot = ConstructSnapshot();
                RpcServerToClientSync(new TransformTargetData(OriginShift.LocalOffset, snapshot.position, snapshot.rotation, IsOffsetChanged()));
            ...
            if (clientAuthority && !hasAuthority)
            {
                ...
                if (SnapshotInterpolation.Compute(
                    ...
                    Interpolate,
                    out OSNTSnapshot computed))
                {
                    OSNTSnapshot start = serverBuffer.Values[0];
                    OSNTSnapshot goal = serverBuffer.Values[1];
        ...
        void UpdateClient()
        {
            ...
            if (IsClientWithAuthority)
            {
                ...
                if (NetworkTime.localTime >= lastClientSendTime + sendInterval)
                {
                    ...
                    // receiver gets it from batch timestamp to save bandwidth.
                    OSNTSnapshot snapshot = ConstructSnapshot();
                    CmdClientToServerSync(new TransformTargetData(OriginShift.LocalOffset, snapshot.position,
            ...
            else
            {
                ...
                if (SnapshotInterpolation.Compute(
                    ...
                    out OSNTSnapshot computed))
                {
                    OSNTSnapshot start = clientBuffer.Values[0];
                    OSNTSnapshot goal = clientBuffer.Values[1];
                    ApplySnapshot(start, goal, computed);
                }
            }
        }
    ...
    public struct OSNTSnapshot : Snapshot
    {
        ...
        public OriginShift.Offset remoteOffset;

        public Vector3 position;
        public Quaternion rotation;

        public OSNTSnapshot(double remoteTimestamp, double localTimestamp, Vector3 position, Quaternion rotation, OriginShift.Offset remoteOffset)
        {
        ...
        public static OSNTSnapshot Interpolate(OSNTSnapshot from, OSNTSnapshot to, double t)
        {
            // NOTE:
            ...
            return new OSNTSnapshot(
                ...
                Vector3.LerpUnclamped(OriginShift.RemoteToLocal(from.remoteOffset, from.position), OriginShift.RemoteToLocal(to.remoteOffset, to.position), (float) t),
